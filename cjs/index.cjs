...
var anyPass =
/*#__PURE__*/
_curry1_1(function anyPass(preds) {
...
var constructN =
/*#__PURE__*/
_curry2_1(function constructN(n, Fn) {
  if (n > 10) {
...
var ifElse =
/*#__PURE__*/
_curry3_1(function ifElse(condition, onTrue, onFalse) {
...
var invoker =
/*#__PURE__*/
_curry2_1(function invoker(arity, method) {
...
const instanceOf = propEq_1('name');

const wait = ms => new Promise(resolve => setTimeout(resolve, ms));
const until = timestampMs => timestampMs - Date.now();

const typeEq = curry_1((string, thing) => type_1(thing) === string);
const isDeltaSeconds = compose_1(both_1(typeEq('Number'), complement_1(Number.isNaN)), parseInt);
const getHeader = curry_1((header, response) => response.headers.get(header));
...
const isRetryableError = both_1(
  is_1(Error),
  anyPass_1(map_1(instanceOf, knownErrors))
);
const isRetryableStatus = anyPass_1([
 ...
 */
const retry$3 = curry_1((max, fetch) => (...args) => lib(async (bail, tries) => {
    ...
    .then(async res => {
      ...
      if (resetHeader && res.status === 429) {
        isDeltaSeconds(resetHeader)
          ? await compose_1(wait, sec => sec * 1000, parseInt)(resetHeader)
          : await compose_1(wait, until, invoker_1(0, 'getTime'), constructN_1(1, Date))(resetHeader);
      }
      return res;
    })
    .then(when_1(res => isRetryableStatus(res.status) && canRetry, forceRetry))
    .catch(ifElse_1(isRetryableError, raise, bail));
}, {
  retries: max,
  minTimeout: 10,
  factor: 5
